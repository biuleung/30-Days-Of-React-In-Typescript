...
function CatParadise() {
    ...
    useEffect(() => {
        fetchCatsData().then((res) => {
            const data = res.data as {
                life_span: string;
                weight: { metric: string };
                origin: string;
            }[];
            setNumOfBreeds(data.length);

            const weightData = data.map((item) => item.weight.metric);
            setAverageWeight(Number(getAverage(weightData).toFixed(2)));

            const liveData = data.map((item) => item.life_span);
    ...
    return (
        <div className={styled.content}>
            ...
            <p className={styled.catsStats + ' d-flex align-items-center'}>
